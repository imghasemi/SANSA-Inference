...
class EREntitySerializer(sc: SparkContext) {
  ...
  def apply(dataPath: String, functionalKeys: EREntitySerializerSemanticResolutionSet): RDD[(Node, Iterable[Node])] = {
    ...
    val entityFragment1 = functionalKeys.entityFragment1
    ...
    def filterFunctionalKeysTriples(t: Triple): Boolean = {
      (t.getPredicate.getURI == rdfTypeURI && t.getObject.toString() == typeOfEntityURI) ||
    ...
    val sameAsTriples = cachedRDDGraph
      .filter(t => (t.p.getURI == rdfTypeURI && t.o.toString() == typeOfEntityURI) || t.getPredicate.getURI == entityFragment1 || t.getPredicate.getURI == entityFragment2)
...
object EREntitySerializerTest {
  ...
  def main(args: Array[String]) {
    ...
    val addressFunctionalKeysRULE2 = EREntitySerializerSemanticResolutionSet("http://datasource2.org/Location", "http://datasource2.org/inCity", "http://datasource2.org/street")
